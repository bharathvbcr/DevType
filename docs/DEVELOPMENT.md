...
**Re-releasing a published tag.** `Scripts/publish-release.sh` refuses to touch a release that is already public, so a rerun or a second publisher bounces off it. A hot patch that has to keep its version is the one deliberate exception: `DEVTYPE_ALLOW_REPUBLISH=1` replaces the notes and the DMG in place, still behind the in-flight-workflow guard, and still ending on the same download-and-compare verification — the hatch relaxes *who may write*, never *whether the result is proven*. It prints what it is doing to a public release, because everyone who already downloaded that version ends up with different bytes under the same version string and `CFBundleVersion` is the only tell. Prefer a new patch version; reach for this only when the version itself must not move.
