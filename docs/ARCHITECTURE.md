...
Host accessibility calls can fail when applications terminate or expose incomplete state. AppKit window and KVC operations can also raise Objective-C exceptions. DevType implements:

1. **`DevTypeSafety` Objective-C Gateways**: Window presentation and KVC assignment use exception-catching wrappers. Accessibility call sites inspect AX return codes and validate returned values; the bridge does not wrap every AX or pasteboard call.
...
When a typed-trigger erase disagrees with `AXValue`, `EraseExecutor` can corroborate the exact UTF-16 erase window through the existing `SelectionReader` range API (`AXStringForRange`, then its attributed form). Recovery requires a collapsed selection that remains unchanged across the read, matching text and unit counts, and a range no larger than 32,768 UTF-16 units. Conflicting views permit only HID erasure; missing or disagreeing range evidence still refuses. Undo, voice, and erasure after a possible write retain their stricter guards. Refusals include a content-free `rangeProbe` status.

Type-ahead retention has both a grapheme capacity and a 4,096 UTF-16-unit payload ceiling, so a burst of combining marks cannot bypass the storage bound. Overflow flushes admitted text exactly once and passes the current event through. Nonfinite hold intervals use the finite default; negative layout-buffer capacities behave as zero capacity.
...
- **Serial delivery (`VoiceInsertionService`)**: one live edit settles before the next diff is computed; revisions arriving meanwhile update the bounded assembler. Final delivery joins pending live work and maps the real injection outcome into a receipt. Failed/refused operations cannot become an inserted outcome. Late callbacks cannot mutate a replacement session, and an uncertain partial post stops further automatic writes. Cancellation with a pending proposal preserves text instead of guessing at a compensating erase. Main-queue delivery reservations are separately bounded to 1 MiB and 4,096 callbacks, and check the captured session generation when dispatched.
...
- A loaded XCTest bundle redirects default support paths to one unique temporary directory per test process, including release-mode tests. Shared voice diagnostics, recovery files and AX learning sidecars therefore use test storage rather than the installed app's support directory. Explicit fixture paths remain under the fixture's control; this does not redirect Keychain or arbitrary external paths.
- `SingleFlightLatch` is shared by AI operations that refuse overlapping work. Its bounded runner serves palette routing and Foundation Models voice correction: timeout or caller cancellation returns after a 50 ms cleanup grace, while cancellation-ignoring work retains admission until it exits. Invalid deadlines admit no work; finite deadlines are capped at 60 seconds. `LocalCorrectorProbe` shares the local correctors' bounded readiness request and typed unavailable/configuration outcomes, while provider adapters retain their route and capability choices.
- `BoundedProcess` drains diagnostic tool output without background reader tasks, caps retained output at 1 MiB by default, and bounds waits for process termination and inherited pipe closure. `ProcessIdentity` accepts only successful, complete, nontruncated output; partial probes remain unavailable.
...
Search admission uses `SnippetSearch.QueryIssue` from the same parser as matching. UTF-8 and term overflow reject the complete query before matching; incomplete negative filters reject too. `CommandPaletteCatalog.buildRows` validates before normalization or cache lookup, and both native search surfaces display the typed reason. Explicit `>` command queries retain their command grammar.
