...
Local release candidate. Public distribution remains gated on Developer ID signing, notarization, and the platform checks recorded in the [audit](../audits/2026-09-10-productivity-release.md).
...
Search remains local. Body matching covers the first 2,000 characters of each snippet; queries process at most 12 terms and 4,096 UTF-8 bytes. The palette also retains its 512-character input limit. Unknown prefixes remain searchable text. Field filters, quoted phrases and exclusions are literal; ordinary positive terms retain fuzzy matching.
...
- Naming conversion preserves non-Latin letters and accents. It converts naming styles; it does not transliterate text or guarantee a valid identifier for every programming language.
