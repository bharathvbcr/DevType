...
- Delayed paste, modifier-gap callbacks, cursor positioning and trailing keys share one operation lifetime. Cancellation, supersession, changed target field/selection, user input and live permission checks can stop later actions. Command is released even when a paste is cancelled during the modifier gap.
